...
/*
 * Bicycle chain geometry
 *
 * A bicycle chain has a 1/2-inch pitch. Rather than treating the
 * chainring and rear cog as smooth pulleys, model the roller centres
 * as the vertices of regular polygons.
 *
 * Adjacent vertices are therefore exactly one chain pitch apart.
 */
const CHAIN_PITCH_IN = 0.5;

/*
 * Number of positions sampled during one tooth-engagement cycle.
 *
 * 32 is already plenty for bicycle-sized sprockets. Increase to 64
 * if desired; it costs additional CPU time.
 */
...
/*
 * Exact pitch radius of a sprocket.
 *
 * The chord between adjacent roller centres is one chain pitch:
 *
 *     pitch = 2 R sin(pi / teeth)
 *
 * therefore:
 *
...
/*
 * Build the pitch polygon for one sprocket.
 *
 * Each point represents the centre of a chain roller sitting in a
 * tooth position.
 */
function buildPitchPolygon(
  teeth,
  centerX,
  phase,
  owner
...
/*
 * 2-D cross product used by the convex-hull solver.
 */
...
/*
 * Andrew's monotonic-chain convex hull.
 *
 * The taut chain envelope around the two sprocket pitch polygons is
 * the convex hull of their roller-centre points.
 */
function convexHull(points) {
  ...
  /*
   * Last point of each list is the first point of the other list.
   */
  lower.pop();
  upper.pop();
...
/*
 * Calculate the length of the taut polygonal chain envelope for one
 * rotational phase of the drivetrain.
 */
function polygonChainPathForPhase(
  ...
  /*
   * phaseFraction represents movement through one chain pitch.
   *
   * A one-pitch movement rotates each sprocket by one tooth.
   */
  const ringPhase =
    phaseFraction *
    2 * Math.PI / chainring;
...
/*
 * Return the tightest chain-envelope length encountered as the
 * drivetrain moves through one tooth-engagement cycle.
 *
 * Using the maximum matters: setting the chainstay from an easier
 * phase could cause the chain to bind when the sprockets rotate into
 * their tightest polygonal phase.
 */
function chainPath(
  chainring,
  cog,
  ...
  for (
    ...
    if (Number.isFinite(path)) {
      maximumPath =
        Math.max(maximumPath, path);
    }
  }

  return Number.isFinite(maximumPath)
    ? maximumPath
    : NaN;
}
...
/*
 * Find the centre distance where the selected chain length becomes
 * taut at the tightest polygonal phase.
 */
